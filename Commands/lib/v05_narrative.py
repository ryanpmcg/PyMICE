"""Original prose from Vignettes/05_multilevel_data/vignette.html."""
...
_PROSE = _ROOT / "Vignettes" / "05_multilevel_data" / "vignette_prose.json"
...
STEP_TITLES: dict[int, str] = {
    ...
    21: (
        "Impute the variable `popular` by means of `2l.norm`. Use dataset `popNCR2`."
    ),
    ...
    25: (
        "Evaluate the imputations by means of convergence, distributions and plausibility."
    ),
    26: (
        "Repeat the same imputations as in the previous step, but now use `pmm` for everything."
    ),
}
...
N4_BEFORE_IND = "In R the missingness indicator"
N4_AFTER_HIST = (
...
N15_AFTER = (
    "The latter case results in a conditional plot (conditional on the different imputed "
    "datasets)."
)
