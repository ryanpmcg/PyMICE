"""Original prose from Vignettes/01_ad_hoc_and_mice/vignette.html."""
...
_PROSE = _ROOT / "Vignettes" / "01_ad_hoc_and_mice" / "vignette_prose.json"
...
def load_step_title(num: int) -> str:
    if num in STEP_TITLES:
        return STEP_TITLES[num]
    data = json.loads(_PROSE.read_text(encoding="utf-8"))
    for step in data["steps"]:
        if int(step["number"]) == num:
            return step["title"]
    raise KeyError(f"No vignette title for step {num}")
